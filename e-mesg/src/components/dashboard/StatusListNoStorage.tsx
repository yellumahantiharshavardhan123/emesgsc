...
import { Plus, Eye, Clock, Upload } from 'lucide-react';
...
export const StatusListNoStorage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe1 = onSnapshot(statusesQuery, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data.content.type === 'text') { // Only include text statuses
          statusList.push({ id: doc.id, ...data } as Status);
        }
      });
    ...
    const unsubscribe2 = onSnapshot(myStatusesQuery, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data.content.type === 'text') { // Only include text statuses
          myStatusList.push({ id: doc.id, ...data } as Status);
        }
      });
        ...
        <div className="space-y-2">
          ...
          <div className="p-3 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <div className="flex items-center justify-center space-x-2 text-gray-400">
              ...
              <span className="text-sm">Photo uploads require Firebase Storage</span>
            </div>
            <p className="text-xs text-gray-500 text-center mt-1">
              Enable Storage to share photos in status updates
            </p>
          </div>
        </div>
      </div>
